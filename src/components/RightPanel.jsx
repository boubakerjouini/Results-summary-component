  ...
  return (
    <div className="bg-white w-full text-left  py-8 sm:rounded-3xl">
      ...
      {data.map((item, index) => {
        return (
          <div
            className={`flex flex-row gap-2 ml-8 mt-6   ${bgColors[index]}  w-[250px] h-10 py-1 px-4 rounded-md align-middle items-center`}
            key={index}
          >
            <img src={icons[index]} alt="" className="w-4 h-4 mt-1 font-extrabold" />
            <h1 className={` ${textsColors[index]} text-blue font-hanken font-medium text-[16px]`}>{item.category}</h1>
          </div>
        )
      })}

      <div className="flex flex-col justify-center items-center my-5">
        <button className="bg-black w-[250px]  h-12 my-12 mt-2 rounded-full text-white font-hanken font-medium text-lg hover:bg-secondary-light">
          Continue
        </button>
      </div>
    </div>
